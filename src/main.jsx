import React, { useEffect, useMemo, useRef, useState } from 'react';
import { createRoot } from 'react-dom/client';
import './styles.css';
import { analyzeImpact } from './topology/impact';
import {
  loadOutageRecords,
  saveOutageRecords,
  outOfServiceIds,
  recordForNode,
  createOutageRecord,
  updateOutagePlan,
  executeOutage,
  restoreOutage,
  cancelOutage,
  recheckOutage,
} from './topology/outageRecords';
import { OutagePanel, OutageRecordsList } from './components/OutagePanel';

const seed = {
  nodes: [
    { id: 'gw', name: '核心路由器', type: 'router', x: 470, y: 220, ip: '10.0.0.1' },
    { id: 'sw1', name: '交换机 A', type: 'switch', x: 250, y: 370, ip: '10.0.1.1' },
    { id: 'sw2', name: '交换机 B', type: 'switch', x: 690, y: 370, ip: '10.0.2.1' },
    { id: 'web', name: 'Web Server', type: 'server', x: 100, y: 520, ip: '10.0.1.10' },
    { id: 'db', name: 'Database', type: 'server', x: 400, y: 550, ip: '10.0.1.20' },
    { id: 'user', name: '办公终端', type: 'device', x: 820, y: 530, ip: '10.0.2.22' },
  ],
  edges: [
    ['gw', 'sw1'],
    ['gw', 'sw2'],
    ['sw1', 'web'],
    ['sw1', 'db'],
    ['sw2', 'user'],
  ],
};

const loadTopology = () => {
  try {
    return JSON.parse(localStorage.getItem('topology')) || seed;
  } catch {
    return seed;
  }
};

const TYPE_ICON = { router: '◉', switch: '▦', server: '▣', device: '▱' };

function App() {
  const [data, setData] = useState(loadTopology);
  const [records, setRecords] = useState(loadOutageRecords);
  const [selected, setSelected] = useState('gw');
  const [tool, setTool] = useState('select');
  const [tab, setTab] = useState('props'); // props | outage | records
  const [notice, setNotice] = useState('');
  const [drag, setDrag] = useState(null);
  const board = useRef();

  // 拓扑与停役记录分别持久化，重新打开页面可继续核对
  useEffect(() => localStorage.setItem('topology', JSON.stringify(data)), [data]);
  useEffect(() => saveOutageRecords(records), [records]);

  const node = data.nodes.find((n) => n.id === selected) || data.nodes[0];

  // 停役中设备集合（由记录推导，单一事实来源）
  const outIds = useMemo(() => outOfServiceIds(records), [records]);
  const selectedRecord = useMemo(
    () => (node ? recordForNode(records, node.id) : null),
    [records, node]
  );

  // 选中设备且无生效记录时，按当前连线实时做影响分析
  const liveImpact = useMemo(
    () => (node && !selectedRecord ? analyzeImpact(data, node.id, outIds) : null),
    [data, node, selectedRecord, outIds]
  );

  // 停役影响标签页打开时，画布按影响结果着色
  const tint = useMemo(() => {
    if (tab !== 'outage') return {};
    const impact = selectedRecord?.impact || liveImpact;
    if (!impact) return {};
    const map = { [impact.targetId]: 'target' };
    impact.noExitNodes.forEach((n) => (map[n.id] = 'noexit'));
    impact.lostTerminals.forEach((t) => (map[t.id] = 'lost'));
    impact.backupDevices.forEach((d) => {
      if (!map[d.id]) map[d.id] = 'backup';
    });
    return map;
  }, [tab, selectedRecord, liveImpact]);

  // ---------- 拓扑编辑（原有功能） ----------
  const updateNode = (k, v) =>
    setData({ ...data, nodes: data.nodes.map((n) => (n.id === selected ? { ...n, [k]: v } : n)) });

  const addNode = (type = 'device', label = '新设备') => {
    const id = 'node' + Date.now();
    setData({
      ...data,
      nodes: [...data.nodes, { id, name: label, type, x: 500, y: 320, ip: '192.168.0.10' }],
    });
    setSelected(id);
    setTool('select');
    setNotice('已添加设备');
  };

  const connect = () => {
    if (!selected) return;
    const other = prompt('输入要连接的设备 ID（例如 sw1）');
    if (
      other &&
      data.nodes.some((n) => n.id === other) &&
      other !== selected &&
      !data.edges.some(
        (e) => (e[0] === selected && e[1] === other) || (e[1] === selected && e[0] === other)
      )
    ) {
      setData({ ...data, edges: [...data.edges, [selected, other]] });
      setNotice('连接已创建');
    }
  };

  const remove = () => {
    if (recordForNode(records, selected)) {
      setNotice('该设备存在待确认或停役中的记录，请先取消或恢复后再删除');
      return;
    }
    setData({
      ...data,
      nodes: data.nodes.filter((n) => n.id !== selected),
      edges: data.edges.filter((e) => !e.includes(selected)),
    });
    setSelected(data.nodes.find((n) => n.id !== selected)?.id);
    setNotice('设备已删除');
  };

  const save = () => setNotice('拓扑图已保存');

  const exportJson = () => {
    const a = document.createElement('a');
    a.href = URL.createObjectURL(
      new Blob([JSON.stringify({ ...data, outageRecords: records }, null, 2)], {
        type: 'application/json',
      })
    );
    a.download = 'network-topology.json';
    a.click();
    setNotice('JSON 已导出（含停役记录）');
  };

  const validate = () => {
    const linked = new Set(data.edges.flat());
    const isolated = data.nodes.filter((n) => !linked.has(n.id));
    setNotice(isolated.length ? `发现 ${isolated.length} 个孤立节点` : '拓扑检查通过：没有孤立节点');
  };

  const move = (e) => {
    if (!drag) return;
    const r = board.current.getBoundingClientRect();
    setData({
      ...data,
      nodes: data.nodes.map((n) =>
        n.id === drag
          ? { ...n, x: Math.max(35, e.clientX - r.left), y: Math.max(35, e.clientY - r.top) }
          : n
      ),
    });
  };

  // ---------- 停役流程 ----------
  const handleCreate = () => {
    const impact = analyzeImpact(data, selected, outIds);
    const { records: next, record, error } = createOutageRecord(records, impact, data);
    if (error) return setNotice(error);
    setRecords(next);
    setNotice(
      (impact.lostTerminals.length
        ? `存在 ${impact.lostTerminals.length} 个失联终端，待确认：请填写替代线路与回退时刻`
        : '已生成停役待确认单') + `：${record.nodeName}`
    );
  };

  const handleExecute = (id) => {
    const { records: next, error } = executeOutage(records, id);
    if (error) return setNotice(error);
    setRecords(next);
    setNotice('已执行停役：设备标记为停役，原连接保留');
  };

  const handleRestore = (id) => {
    const { records: next, error, edgesToAdd, warning } = restoreOutage(records, id, data);
    if (error) return setNotice(error);
    if (edgesToAdd.length) setData({ ...data, edges: [...data.edges, ...edgesToAdd] });
    setRecords(next);
    setNotice(warning || `已恢复，照原线接回 ${edgesToAdd.length} 条连接`);
  };

  const handleCancel = (id) => {
    setRecords(cancelOutage(records, id));
    setNotice('停役待确认单已取消');
  };

  const handleRecheck = (id) => {
    const { records: next, result } = recheckOutage(records, id, data, outIds);
    setRecords(next);
    if (result) setNotice(result.same ? '核对通过：与停役前分析一致' : result.summary);
  };

  const locateNode = (id) => {
    setSelected(id);
    setTab('outage');
  };

  // ---------- 渲染 ----------
  return (
    <div className="app">
      <header>
        <div className="brand">
          <span className="brand-mark">⌁</span>
          <div>
            <strong>NETSCAPE</strong>
            <small>TOPOLOGY STUDIO · 停役影响台</small>
          </div>
        </div>
        <div className="file">
          <span className="dot"></span>
          <div>
            <strong>office-network.json</strong>
            <small>最近保存：刚刚</small>
          </div>
        </div>
        <div className="top-actions">
          <button onClick={validate}>✓ 检查</button>
          <button onClick={exportJson}>↓ 导出</button>
          <button className="save" onClick={save}>
            保存更改
          </button>
        </div>
      </header>

      <div className="toolbar">
        <div className="tool-group">
          <span>工具</span>
          <button className={tool === 'select' ? 'on' : ''} onClick={() => setTool('select')}>
            ↖ 选择
          </button>
          <button
            className={tool === 'connect' ? 'on' : ''}
            onClick={() => {
              setTool('connect');
              connect();
            }}
          >
            ⌁ 连接
          </button>
          <button onClick={() => addNode()}>＋ 设备</button>
        </div>
        <div className="tool-group zoom">
          <button>−</button>
          <span>100%</span>
          <button>＋</button>
          <button onClick={() => setNotice('画布已居中')}>⌗</button>
        </div>
      </div>

      <div className="workspace">
        <aside className="inventory">
          <div className="section-title">
            <span>设备库</span>
            <small>{data.nodes.length} 个节点</small>
          </div>
          <div className="device-types">
            {[
              ['router', '◉', '路由器'],
              ['switch', '▦', '交换机'],
              ['server', '▣', '服务器'],
              ['device', '▱', '终端设备'],
            ].map(([t, i, l]) => (
              <button onClick={() => addNode(t, l)} key={t}>
                <i className={t}>{i}</i>
                {l}
                <span>＋</span>
              </button>
            ))}
          </div>
          <div className="section-title nodes-head">
            <span>图中节点</span>
            <small>点击查看</small>
          </div>
          <div className="node-list">
            {data.nodes.map((n) => (
              <button
                className={selected === n.id ? 'sel' : ''}
                onClick={() => setSelected(n.id)}
                key={n.id}
              >
                <i className={n.type}>{TYPE_ICON[n.type]}</i>
                <span>
                  <strong>{n.name}</strong>
                  <small>{n.ip}</small>
                </span>
                {outIds.has(n.id) && <em className="oos-tag">停役</em>}
                <b>›</b>
              </button>
            ))}
          </div>
        </aside>

        <section className="canvas-wrap">
          <div className="canvas" ref={board} onMouseMove={move} onMouseUp={() => setDrag(null)}>
            {data.edges.map(([a, b], i) => {
              const n1 = data.nodes.find((n) => n.id === a);
              const n2 = data.nodes.find((n) => n.id === b);
              if (!n1 || !n2) return null;
              const dx = n2.x - n1.x;
              const dy = n2.y - n1.y;
              const len = Math.hypot(dx, dy);
              const ang = (Math.atan2(dy, dx) * 180) / Math.PI;
              const retained = outIds.has(a) || outIds.has(b);
              const cutting =
                tab === 'outage' && !retained && (tint[a] === 'target' || tint[b] === 'target');
              return (
                <div
                  className={'edge' + (retained ? ' retained' : '') + (cutting ? ' cutting' : '')}
                  key={i}
                  style={{ left: n1.x, top: n1.y, width: len, transform: `rotate(${ang}deg)` }}
                >
                  <span></span>
                </div>
              );
            })}
            {data.nodes.map((n) => (
              <button
                className={
                  'node ' +
                  n.type +
                  (selected === n.id ? ' picked' : '') +
                  (outIds.has(n.id) ? ' out-of-service' : '') +
                  (tint[n.id] ? ` tint-${tint[n.id]}` : '')
                }
                style={{ left: n.x - 42, top: n.y - 31 }}
                onMouseDown={(e) => {
                  e.stopPropagation();
                  setSelected(n.id);
                  setDrag(n.id);
                }}
                onClick={() => setSelected(n.id)}
                key={n.id}
              >
                <i>{TYPE_ICON[n.type]}</i>
                <strong>{n.name}</strong>
                <small>{outIds.has(n.id) ? '停役中' : n.ip}</small>
              </button>
            ))}
            <div className="legend">
              <span>
                <i className="router"></i>路由器
              </span>
              <span>
                <i className="switch"></i>交换机
              </span>
              <span>
                <i className="server"></i>服务器
              </span>
              {tab === 'outage' && (
                <>
                  <span>
                    <i className="lg-lost"></i>失联终端
                  </span>
                  <span>
                    <i className="lg-backup"></i>备用线路
                  </span>
                  <span>
                    <i className="lg-noexit"></i>无出口
                  </span>
                </>
              )}
            </div>
          </div>
          <div className="canvas-footer">
            <span>
              拖动节点调整位置 · {data.edges.length} 条连接
              {outIds.size > 0 && ` · ${outIds.size} 台停役中`}
            </span>
            <span>坐标系：画布局部</span>
          </div>
        </section>

        <aside className="inspector">
          <div className="tabs">
            <button className={tab === 'props' ? 'on' : ''} onClick={() => setTab('props')}>
              属性
            </button>
            <button className={tab === 'outage' ? 'on' : ''} onClick={() => setTab('outage')}>
              停役影响
            </button>
            <button className={tab === 'records' ? 'on' : ''} onClick={() => setTab('records')}>
              记录{records.length > 0 && <em>{records.length}</em>}
            </button>
          </div>

          {tab === 'props' &&
            (node ? (
              <>
                {outIds.has(node.id) && (
                  <div className="oos-banner">该设备停役中，连线保留但不参与连通</div>
                )}
                <label>
                  设备名称
                  <input value={node.name} onChange={(e) => updateNode('name', e.target.value)} />
                </label>
                <label>
                  IP 地址
                  <input value={node.ip} onChange={(e) => updateNode('ip', e.target.value)} />
                </label>
                <label>
                  设备类型
                  <select value={node.type} onChange={(e) => updateNode('type', e.target.value)}>
                    <option value="router">路由器</option>
                    <option value="switch">交换机</option>
                    <option value="server">服务器</option>
                    <option value="device">终端设备</option>
                  </select>
                </label>
                <div className="inspector-actions">
                  <button onClick={connect}>⌁ 添加连接</button>
                  <button className="danger" onClick={remove}>
                    删除设备
                  </button>
                </div>
                <div className="connections">
                  <div className="section-title">
                    <span>连接</span>
                    <small>{data.edges.filter((e) => e.includes(node.id)).length} 条</small>
                  </div>
                  {data.edges
                    .filter((e) => e.includes(node.id))
                    .map((e, i) => {
                      const other = data.nodes.find((n) => n.id === (e[0] === node.id ? e[1] : e[0]));
                      return (
                        <div className="connection" key={i}>
                          <span className={'mini ' + other?.type}></span>
                          <strong>{other?.name}</strong>
                          <small>{other && outIds.has(other.id) ? '停役' : '在线'}</small>
                        </div>
                      );
                    })}
                </div>
              </>
            ) : (
              <p>选择一个设备</p>
            ))}

          {tab === 'outage' && (
            <OutagePanel
              node={node}
              record={selectedRecord}
              liveImpact={liveImpact}
              notice={notice}
              onCreate={handleCreate}
              onUpdatePlan={(id, patch) => setRecords(updateOutagePlan(records, id, patch))}
              onExecute={handleExecute}
              onRestore={handleRestore}
              onCancel={handleCancel}
              onRecheck={handleRecheck}
            />
          )}

          {tab === 'records' && (
            <OutageRecordsList records={records} nodes={data.nodes} onLocate={locateNode} />
          )}
        </aside>
      </div>

      {notice && <div className="toast">{notice}</div>}
    </div>
  );
}

createRoot(document.getElementById('root')).render(<App />);
